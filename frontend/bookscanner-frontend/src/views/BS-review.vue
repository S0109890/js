        ...
        <b-row no-gutters>
          ...
          <b-col id="review-data" md="8">
            <b-card-body
              ...
              :sub-title="`${storageInfo.author} | ${storageInfo.publisher} | ${storageInfo.price} | ${storageInfo.location}`"
            >
              <b-card-text>
  ...
  methods: {
    ...
    onClickReviewDelete(id) {
      this.$bvModal.msgBoxConfirm('이 중고도서 리뷰를 삭제 하시겠습니까?').then(value => {
        if (value) {
          this.$store.dispatch('actStorageDelete', { ...this.storageInfo, review: null })
        } else {
          return
        }
      })
